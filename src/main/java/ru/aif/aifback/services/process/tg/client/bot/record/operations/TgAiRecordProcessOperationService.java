...
import static ru.aif.aifback.services.process.tg.client.bot.record.TgClientBotRecordButtons.AI_RECORD_CONFIRM_TITLE;
import static ru.aif.aifback.services.process.tg.client.bot.record.TgClientBotRecordButtons.AI_RECORD_ERROR_TITLE;
import static ru.aif.aifback.services.process.tg.client.bot.record.TgClientBotRecordButtons.AI_RECORD_PROCESS_TITLE;
...
public class TgAiRecordProcessOperationService implements TgClientBotOperationService {
    ...
    public void process(WebhookRequest webhookRequest, UserBot userBot, TelegramBot bot) {
        ...
        Integer waitMessageId = CommonUtils.sendMessage(
        ...
        if (Objects.nonNull(waitMessageId)) {
            CommonUtils.deleteMessage(webhookRequest.getChatId(), waitMessageId, bot);
        }
    ...
    private void processOneStaff(AiRecordStaffResponse staff, Long hours, Long mins, String chatId, int messageId, TelegramBot bot,
        ...
        String answer = "\uD83D\uDD35 <b>Новая запись</b>\n\n" +
                        String.format("\uD83D\uDCC5 <b>Дата:</b> %s %02d %s %s <b>%02d:%02d</b>",
                                      CommonUtils.getDayOfWeek(calendar.getDay(), calendar.getMonth(), calendar.getYear()),
                                      calendar.getDay(),
                                      CommonUtils.getMonthByNumber(calendar.getMonth()),
                                      calendar.getYear(),
                                      hours,
                                      mins) +
                        String.format("\n\n\uD83D\uDCE6 <b>Услуга:</b> %s\n\n", userItem.getName()) +
        ...
        keyboard.addRow(new InlineKeyboardButton(AI_RECORD_CONFIRM_TITLE).callbackData(
                String.format("%s;%s;%s;%s;%s;%s;%s",
                              ClientRecordBotOperationType.BOT_CONFIRM_SELECT_TIME.getType(),
                              calendar.getId(),
                              hours,
                              mins,
                              userItem.getId(),
        ...
        CommonUtils.sendMessage(chatId, messageId, answer, keyboard, bot, TRUE);
    }
    ...
    private void sendErrorSearchRecord(String chatId, int messageId, TelegramBot bot) {
        InlineKeyboardMarkup keyboard = new InlineKeyboardMarkup();
        keyboard.addRow(createBackButton(ClientRecordBotOperationType.BOT_MAIN.getType()));
        CommonUtils.sendMessage(chatId, messageId, AI_RECORD_ERROR_TITLE, keyboard, bot, TRUE);
    }
    ...
    public ClientRecordBotOperationType getOperationType() {
        return ClientRecordBotOperationType.BOT_AI_RECORD_PROCESS;
    }
}
