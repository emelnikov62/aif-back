...
import static ru.aif.aifback.services.process.tg.client.bot.record.TgClientBotRecordButtons.ACTIVE_TITLE;
import static ru.aif.aifback.services.process.tg.client.bot.record.TgClientBotRecordButtons.HISTORY_TITLE;
import static ru.aif.aifback.services.process.tg.client.bot.record.TgClientBotRecordButtons.RECORDS_EMPTY_TITLE;
...
public class TgRecordsOperationService implements TgClientBotOperationService {
    ...
    private String processBotRecords(String clientTgId, String status, InlineKeyboardMarkup keyboard) {
        ...
        return fillClientRecords(keyboard, clientId, status)
               ? (Objects.equals(status, ClientRecordType.NO_ACTIVE.getType()) ? HISTORY_TITLE : ACTIVE_TITLE)
               : RECORDS_EMPTY_TITLE;
    }
    ...
    private Boolean fillClientRecords(InlineKeyboardMarkup keyboard, Long clientId, String status) {
        List<ClientRecord> clientRecords = Objects.equals(status, ClientRecordType.NO_ACTIVE.getType())
        ...
        clientRecords.forEach(clientRecord -> {
            String dayOfWeek = CommonUtils.getDayOfWeek(clientRecord.getUserCalendar().getDay(),
            ...
            keyboard.addRow(new InlineKeyboardButton(String.format("%s \uD83D\uDCC5 %s %02d %s %s %02d:%02d (%s)",
                                                                   ...
                                                                   CommonUtils.getMonthByNumber(clientRecord.getUserCalendar().getMonth()),
                                                                   clientRecord.getUserCalendar().getYear(),
